...
func (r *Repo) Create(ctx  context.Context, note Note) (Note, error) {
	...
	if err != nil {
		return Note{}, fmt.Errorf("Failed to insert note ")
	}
	return note , nil 
}
...
func (r *Repo) List(ctx context.Context) ([]Note, error){
	...
	cursor, error := r.coll.Find(opCtx,filter)

	if error != nil {
		return nil , fmt.Errorf("Failed to fetch notes from db : %s", error)
	}
	...
	if err := cursor.All(opCtx, &notes); err != nil {
		return nil, fmt.Errorf("failed to decode notes: %s ", err)
	}
	return  notes, nil
}
...
func (r *Repo) GetById(ctx context.Context, id primitive.ObjectID)(Note , error){
	...
	if err != nil {
		return Note{}, fmt.Errorf("Failed to find the note %s", err)  
	}
...
func (r *Repo) UpdateById(ctx context.Context , id primitive.ObjectID, req UpdateNoteRequest )(Note , error) {
	...
	var updated Note 
	err := r.coll.FindOneAndUpdate(opCtx,filter, updatedData,&opts).Decode(&updatedData)
	if err != nil {
