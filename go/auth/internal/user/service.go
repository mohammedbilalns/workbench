...
func (s *Service) Register(ctx context.Context, input RegisterInput )(AuthResult, error){
	...
	if len(pass)< 6 {
		return AuthResult{}, errors.New("Password must be atleast 6 chars long")
	}

	_ , err := s.repo.FindByEmail(ctx, email)

	if err == nil {
		return AuthResult{} ,errors.New("Email is already registered, Please try with another email")
	}
